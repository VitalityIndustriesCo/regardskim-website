...
export async function POST(req: NextRequest) {
  try {
    ...
    const honeypot = body.honeypot || ""; // hidden field — bots fill this
    ...
    if (MAILERLITE_API_KEY) {
      const res = await fetch(MAILERLITE_API, {
        ...
        body: JSON.stringify({
          ...
          groups: ["184878108747236964"], // Affiliate Partners group
          fields: {
            name,
            company: website,
          },
      ...
      if (!res.ok) {
        ...
        console.error(`[partner-apply] MailerLite error (${res.status}):`, err);
        // Still return success to user — don't expose backend errors
      }
    }

    console.log(`[partner-apply] ${email}${message ? " (with message)" : ""}`);
    return NextResponse.json({ ok: true });
  } catch (err) {
