...
function ComparisonCard({ title, items, tone }: ComparisonCardProps) {
  ...
  return (
    <article
      className={[
        "h-full rounded-[1.75rem] border bg-paper p-6 shadow-[0_12px_40px_rgba(26,26,26,0.12)] md:p-8",
        isKim ? "border-brass/40 bg-gradient-to-br from-paper via-paper to-cream/70" : "border-forest/15",
      ].join(" ")}
    >
      <div className="flex items-center justify-between gap-4">
        <h3 className="font-display text-2xl font-bold tracking-normal text-forest md:text-3xl">{title}</h3>
        <span
          className={[
            "rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-[0.16em]",
            isKim ? "bg-brass/12 text-brass" : "bg-forest/5 text-slate",
          ].join(" ")}
        >
          ...
          <li key={item} className="flex items-start gap-3">
            <span
              className={[
                ...
                isKim ? "border-brass/20 bg-brass/10 text-brass" : "border-forest/10 bg-forest/5 text-slate",
              ].join(" ")}
            >
              <Icon size={14} strokeWidth={2.4} />
            </span>
            <span className={isKim ? "text-forest" : "text-slate"}>{item}</span>
          </li>
        ))}
      </ul>
    </article>
...
function ReplyCard({ label, body, tone }: ReplyCardProps) {
  ...
  if (!isKim) {
    return (
      <article className="h-full rounded-[1.75rem] border border-forest/15 bg-paper p-6 shadow-[0_12px_40px_rgba(26,26,26,0.12)] md:p-8">
  ...
  return (
    <article className="h-full rounded-[1.75rem] border-2 border-brass/40 bg-gradient-to-br from-paper via-paper to-cream/80 p-6 shadow-[0_16px_44px_rgba(26,26,26,0.14)] md:p-8">
      ...
      <div className="mt-5 rounded-[1.25rem] border border-brass/15 bg-paper/90 p-5 md:p-6">
        ...
        <div className="space-y-4 text-sm leading-7 text-forest md:text-base">
          ...
          <a
            href="#"
            className="inline-flex items-center rounded-full border border-brass/25 bg-brass/10 px-4 py-2 text-sm font-semibold text-brass transition hover:bg-brass/15"
          >
            View tracking →
          </a>
  ...
  return (
    <section className="bg-paper py-16 md:py-24">
      <div className="section-shell">
        ...
        <FadeIn delay={0.12}>
          <div className="mx-auto mt-10 max-w-4xl rounded-[1.75rem] border border-forest/15 bg-paper px-6 py-5 text-center shadow-[0_12px_40px_rgba(26,26,26,0.12)] md:px-8 md:py-6">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate">Customer email</p>
            <p className="mt-3 text-base leading-7 text-forest md:text-lg">{customerEmail}</p>
          </div>
        </FadeIn>
